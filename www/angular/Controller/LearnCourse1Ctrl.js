    ...
    .controller("LearnCourse1Ctrl", ["$scope", "$interval", "$stateParams", "dataService",
        ...
        function ($scope, $interval, $stateParams, dataService, convertService,
            ...
            $scope.getContent = function (courseItem) {
                ...
                var defer = $q.defer();
                if (courseItem.file.indexOf(".txt") < 0) defer.resolve("");
            ...
            $scope.createVideo = function (item, video, index) {
                return;
                $timeout(function () {
                    try{
            ...
            var createSwiper = function() {
                m_swiper = new Swiper('.swiper-container', {
                    ...
                    onSlideChangeEnd: function (swiper) {
                        ...
                        scope.createVideo(item, item.video, swiper.activeIndex);
                        setLockSwipe();
                        addSwiper(swiper);
                    }
                });
            ...
            var setEventListenerEx = function (index) {
                $("span[id*='questionSpan" + index + "']").unbind("click").click(function () {
                    var id = this.id;
                    var checkId = "#" + id.replace("questionSpan", "questionCheck");
            ...
            var selectAnswerEx = function () {
                ...
                $(id).each(function (index, element) {
                    var checkValue = $(element).attr("checked");
                    if (checkValue) {
            ...
            var getCatalogHtml = function(item) {
                ...
                for (var i = 0; i < item.items.length; i++) {
                    if (item.items[i].title.indexOf("图片") >= 0 ||
                        ...
                        continue;
                    html += '<div class="leftIndentation padding" ><span>' + item.items[i].title + '</span></div>';
                }
            ...
            var getQuestionHtml = function(item, index) {
                ...
                for (var i = 0; i < titleList.length; i++) {
                    ...
                    if (titleList[i].trim() == "") {
                        titleList = titleList.splice(i, 1);
                    }
                }
                ...
                for (var i = 0; i < titleList.length; i++) {
                    html += '<span style="display:block; margin-top:8px;">' + titleList + '</span>';
                }
                html += '</strong>';
                html += '</div>';
            ...
            var getCourseResourceHtml = function(item) {
                ...
                if (item.fileType == "img") {
                    ...
                    for (var i = 0; i < item.TextContent.length; i++) {
                        html += '<div class="item-text-wrap padding">';
